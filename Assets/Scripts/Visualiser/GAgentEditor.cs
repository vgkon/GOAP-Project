...
public class GAgentVisualEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("Current Action: " + agent.gameObject.GetComponent<GAgent>().curAction);
        ...
        foreach (KeyValuePair<SubGoal, int> g in agent.gameObject.GetComponent<GAgent>().goals)
        {
            ...
            foreach (KeyValuePair<string, int> sg in g.Key.sGoals)
                GUILayout.Label("=====  " + sg.Key + " : " + g.Value);
        }
        ...
        foreach (GameObject g in agent.gameObject.GetComponent<GAgent>().inventory.GetItems())
        {
